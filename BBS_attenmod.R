...
#numses=#total number of sessions #not used in script as some evals over 2 days others over 3 (and we don't need it)
...
#write as a TSV
write_tsv(POMSDesc, file.path(outputdir, "task-poms_stats.tsv"))

#make new df with Time, POMS
forms=c(POMS$TMD1.1, POMS$TMD1.2, POMS$TMD2.1, POMS$TMD2.2,
        POMS$TMD3.1, POMS$TMD3.2)
time=rep(c(1,2,3,4,5,6),each= numsubs)
day=rep(c(1,2,3), each=2*numsubs)
sub=rep(POMS$Sub, 6)
test=c(rep("POMS", 6*numsubs))
POMSlong=tibble("Subject"=sub, "Task"=test, "Score"=forms, "Timepoint"=time, "Day"=day)
str(POMSlong)
POMSlong=within(POMSlong, {
        Task=factor(Task)
